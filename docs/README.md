...
**fantuan v2.0** 是一个用 **x86-64 汇编语言**编写的编译器，它将 **自定义 CPU 架构下的二进制指令** 转换为 **NASM 汇编源代码**（`output.asm`）。该编译器使用纯汇编实现，支持 **Windows (PE32+)** 和 **Linux (ELF64)** 双平台构建。
